...
class ResUNet(nn.Module):
    def __init__(self, InChannel, OutChannel, base=config.Base, depth=config.Depth, mode='seg', IncludeSeg=True):
        """
...
class PAN(nn.Module):
    def __init__(self, base=config.Base, InChannel=config.InputChannel, OutChannel=1, BackBone='FPN', GoDown=True,
                 IncludeTop=True,
                 IncludeSeg=True):
        ...
        self.BackBone = FPN(base=base, InChannel=InChannel,
                            OutChannel=OutChannel, IncludeSeg=IncludeSeg) if BackBone.lower() == 'fpn' else ResUNet(
            base=base,
            InChannel=InChannel,
            OutChannel=OutChannel,
            mode='det', IncludeSeg=IncludeSeg)
        if GoDown:
...
class RetinaUNet(BasicModule):
    def __init__(self, base=config.Base, InChannel=config.InputChannel, OutChannel=1, OutLayer=config.Output_features,
        ...
        self.MainNet = PAN(base, InChannel, OutChannel, BackBone=BackBone, IncludeTop=IncludeTop, IncludeSeg=IncludeSeg,
                           GoDown=Godown)
        self.ANCNets = nn.ModuleList([AnchorNet(256, 256) for _ in range(self.OutLayer)])
        self.REGNets = nn.ModuleList([BOXRegression(256, 256) for _ in range(self.OutLayer)])

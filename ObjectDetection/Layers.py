...
class AnchorNet(nn.Module):
    def __init__(self, InChannels, OutChannels):
        ...
        self.Blocks = nn.Sequential(
            ...
            # nn.ReLU(inplace=True),
            Mish(),
            nn.Conv2d(OutChannels, self.num_anchors, kernel_size=1, stride=1, bias=True)
        )
...
class UpBottomPath(nn.Module):
    ...
    def forward(self, features):
        if len(features) != self.numBlocks + 1:
            raise ValueError(
                "features length {}, Blocks Length {}, Not Match".format(len(features), self.numBlocks + 1))
...
if __name__ == '__main__':
    ...
    print(model)
    features = [t.rand(1, 64, 256, 256).cuda(), t.rand(1, 256, 128, 128).cuda(), t.rand(1, 512, 64, 64).cuda(),
                t.rand(1, 1024, 32, 32).cuda(), t.rand(1, 2048, 16, 16).cuda()]
    # features.reverse()

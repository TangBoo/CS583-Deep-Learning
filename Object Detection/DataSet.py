...
class RetinaDataSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return img / 255, mask / 255, anchors, bbox_labels, ancLossIdx, bboxLossIdx, seg
...
def test():
    ...
    # idx = 207
    input_dict = dataset[idx]

    output_anchcors, output_bboxs = apply_bboxReg_area(bboxMat=input_dict["bbox_labels"], img_shape=config.Image_shape,
                                                       anchors=input_dict["anchors"])
    img = input_dict["image"].transpose(1, 2, 0)
    print("------------------------Show Image-------------------------------")
    print(idx)
    gt_boxes = []
    expFactor = config.ExpBox
    for i in range(config.BoxScale):
        gt_boxes.append(generator_bbox(input_dict["mask"], mode='box', expFactor=expFactor))
        expFactor += 1
    gt_boxes = np.array(gt_boxes)
    gt_boxes = gt_boxes.reshape((-1, 4))
    plt.figure(idx)

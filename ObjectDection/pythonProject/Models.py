...
from Layers import BottleBlock, UpBottomPath, BottomUpPath, ResidualConv, UpSample
...
class ResNet(nn.Module):
    def __init__(self, base=64, InChannel=1, numBlocks=config.ResNetStructure['ResNet50']):
...
if __name__ == "__main__":
    model = PAN(base=64, InChannel=1, backbone='unet').cuda(0)
    # model = ResUNet(1, 1, 64, 6, mode='Det').cuda(0)

    # summary(model, input_size=(1, 256, 256))
    x = t.randn(1, 1, 128, 128).float().cuda()
    seg, proposals = model(x)
    for idx, f in enumerate(proposals):
        print(idx, f.shape)

...
from utils import Visualizer, features, get_pth, get_ImgPath, data_split_2D, nms, myStr, t2i, read_image_2D, mapAnc2Img
...
def val(model, dataloader, vis, boxShow=False):
    ...
    with torch.no_grad():
        for ii, (img, mask, anchors_labels, bbox_labels, ancLossIdx, bboxLossIdx, segIdx, GtBoxes) in enumerate(
            ...
            image_ = mapAnc2Img(ancOutput, img, threshold=0.6)
            vis.show_anchor(name="Anc Loc", imgs=image_)
...
def train(**kwargs):
    ...
    model = nn.DataParallel(
        RetinaUNet(base=config.Base, InChannel=config.InputChannel, OutChannel=1, BackBone='fpn', IncludeTop=False, Godown=False, IncludeSeg=False)).cuda()
    ...
    for epoch in range(config.Max_epoch):
        ...
        for ii, (img, mask, anchors_labels, bbox_labels, ancLossIdx, bboxLossIdx, segIdx, _) in enumerate(
            ...
            for i in range(config.Output_features):
                ...
                temp_reg = RegCriterion(V(boxOutput[i]).cuda(), V(bbox_labels[i]).cuda(),
                                        V(bboxLossIdx[i]).cuda()).requires_grad_(True)
                if temp_reg != 0 and not t.isnan(temp_reg):
                    box_loss += temp_reg
                # ----------------Anchor Loss Computation----------------
                temp_anc = AncCriterion(V(ancOutput[i]).cuda(), V(anchors_labels[i]).cuda(),
                                        V(ancLossIdx[i]).cuda()).requires_grad_(True)
                if temp_anc != 0 and not t.isnan(temp_anc):
                    anc_loss += temp_anc

                Anc_Matrix.genConfusionMat(V(ancOutput[i]).cuda(), V(anchors_labels[i]).cuda())
            ...
            loss = seg_loss / (ii + 1) + anc_loss + box_loss
            if loss == 0:
        ...
        print(epoch_str + " Time:" + str(end_time - start_time) + ' lr: {}'.format(optimizer.state_dict()['param_groups'][0]['lr']))
...
def test():
    model = nn.DataParallel(RetinaUNet(base=64, InChannel=3, OutChannel=1, BackBone='fpn')).cuda(0)
    vis = Visualizer(env=config.Env, port=config.vis_port)

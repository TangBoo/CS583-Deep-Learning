...
from utils import (create_box, create_idx, point2box, iou_mat,
                   ...
                   generator_bbox, bbox2show, iou, cnt2box)


def generator_anchor_area(img_shape, gtBoxes, depth=config.Depth, area_size=config.Areas, area_ratios=config.Area_ratios,
                          hw_ratios=config.HW_ratio, Pos_threshold=config.PosThreshold, Neg_threshold=config.NegThreshold,
    ...
    recepField = [2 ** i for i in range(0, depth)]
    feature_size = [(h // j, w // j) for j in recepField]  # eg.[3, 5, 9]
    ...
    if area_size is None:
        area_size = [recepField[j] ** 2 for j in config.OutputIndex]
    ...
    for i in range(len(area_size)):
        ...
        bbox_labels.append(bbox_label_)

    return anchors, bbox_labels, indexes


def apply_bboxReg_area(bboxMat, img_shape, anchors, threshold=config.PosThreshold, area_size=config.Areas,
                       area_ratios=config.Area_ratios,
                       hw_ratios=config.HW_ratio):
    """
    ...
    if area_size is None:
        recepField = [2 ** i for i in range(0, config.Depth)]
        area_size = [recepField[j] ** 2 for j in config.OutputIndex]

    proposals = create_box(area_size, area_ratios, hw_ratios)  # [3, 3, 3, 2]
    output_bboxs = []
    output_anchcors = []
    for i in range(len(proposals)):
        f_h, f_w = bboxMat[i].shape[1:-1]
        ...
        if t.is_tensor(anchor_):
            anchor_ = anchor_.numpy()

        if t.is_tensor(output_mat):
            output_mat = output_mat.numpy()

        if output_mat.shape != boxCoord.shape:
        ...
        output_mat[..., 2:3] = boxCoord[..., 2:3] * np.exp(output_mat[..., 2:3])
        output_mat[..., 3:] = boxCoord[..., 3:] * np.exp(output_mat[..., 3:])
        taken_index = np.where(anchor_ > threshold)[:-1]

        # print('anchor shape:', anchor_.shape)
        # print('taken index: ', len(taken_index[0]))
        # print('output shape:', output_mat[taken_index].shape)
...
def test():
    ...
    gt_boxes = generator_bbox(mask, mode='box', expFactor=config.ExpBox)
    img_shape = config.Image_shape if len(config.Image_shape) == 2 else config.Image_shape[1:]
    anchors, bbox_labels, indexes = generator_anchor_area(img_shape=img_shape, gtBoxes=gt_boxes)
